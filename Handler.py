...
from Constants import BARRIER_SIZE
...
class EventHandler:
    ...
    def generation(self, size=200):
        gen = Generation(size, self.screen, self.centre)
        self.world_coord = (size + BARRIER_SIZE * 2) // 2
        gen.generation()
        self.matr = gen.add_barrier(BARRIER_SIZE)
    ...
    def update(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                c = event
                if event.key == pygame.K_ESCAPE and len(self.interfaces) >= 2:
                    try: self.interfaces.pop(self.end)
                    except Exception: pass
                if event.key == pygame.K_ESCAPE and not self.open_some:
        ...
        try:
            for interface in self.interfaces:
                self.end = interface
                self.interfaces[interface].surface.update(self.camera.mouse_click, self.screen, c)
        except Exception:
            pass
